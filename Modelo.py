#Guarda las placas en un archivo csv
import cv2
import numpy as np
import pytesseract
import re
...
def detect_plate(frame, csv_writer):
    ...
    for contour in contours:
        ...
        if len(approx) == 4:
            ...
            # Filtrar placas que siguen el patrón especificado
            plate_pattern = re.compile(r'^[a-zA-Z0-9]{3}-[a-zA-Z0-9]{3}-[a-zA-Z0-9]$')
            if plate_pattern.match(plate_text):
                # Mostrar el texto de la placa en el marco
                cv2.putText(frame, plate_text, (x, y-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                
                # Mostrar el texto de la placa en la consola
                print("Placa detectada:", plate_text)
                
                # Escribir la placa en el archivo CSV
...
with open('placas_detectadas.csv', 'w', newline='') as csvfile:
    ...
    while True:
        ...
        if ret:
            ...
            frame = detect_plate(frame, csv_writer)
